...
export class PickUpComponent implements OnInit {
      ...
      generateExcel() {                        
        ...
        this.pickUpService.getAllTrips().subscribe(data => {
            ...
            for (let index = 0; index < this.jsonObj.length; index++) {
				...
				deletedTp, jTemp.packageTrip.valPackage, jTemp.modeTrip);
   ...
   generatePackwaysReport() {                        
    this.tripsAll = [];        

    let dt = new Date();
    let monthDate = dt.getMonth()+1 + '-' + dt.getFullYear();
    let costByMonth = 0;
    this.pickUpService.generatePackwaysReport(monthDate).subscribe(data => {
        this.result = data['_body'];
        ...
        for (let index = 0; index < this.jsonObj.length; index++) {
            let jTemp = this.jsonObj[index];
            let ctf = Number(jTemp[2]) + Number(jTemp[4]) +Number(jTemp[6]) +Number(jTemp[8]);
    ...
    () => {            
        this.excelService.generatePackwaysExcelReport(this.tripsAll, costByMonth, monthDate);            
    });
  }


  getCaisse(){
    this.pickUpService.getCaisseService().subscribe((data: any) =>{             
        this.caissse = data;            
    })
    setTimeout(() => {
        console.log("thiscasse",this.caissse);
    }, 2000);
