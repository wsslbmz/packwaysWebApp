    ...
    generateExcel() {
        ...
        this.userService.getTripsByUserAndDate(this.idUserTr, this.sDate, this.eDate).subscribe(data => {
            ...
            for (let index = 0; index < this.jsonObj.length; index++) {
                ...
                let tab: any = [];
                tab.push(jTemp.refTrip, jTemp.statusTrip, jTemp.costTrip, jTemp.destTrip.cityAdr, this.splitDateFormatMDY(jTemp.getedday),
                 this.splitDateFormatMDY(jTemp.livredday), cloture, jTemp.packageTrip.valPackage);
