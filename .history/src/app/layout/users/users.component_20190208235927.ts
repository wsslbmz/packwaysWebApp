 ...
 changeDateFormatMDY(dd){
    ...
    generateExcel() {
        ...
        this.userService.getTripsByUserAndDate(this.idUserTr, this.sDate, this.eDate).subscribe(data => {
            ...
            for (let index = 0; index < jTemp2.length; index++) {                

                if (jTemp2.statusTrip === 'Livree' || jTemp2.statusTrip === 'Retour') {
                    if (jTemp2.isClosed === true) {
                        cloture = 'Oui';
                    } else {
                        cloture = 'Non';
                    }
    
                    if (jTemp2.statusTrip === 'Retour') {
                        if (jTemp2.isBilled === true) {
                            jTemp2.packageTrip.valPackage = 0;
                            jTemp2.statusTrip = 'Retour avec frais';
                        } else {
                            jTemp2.costTrip = 0;
                            jTemp2.packageTrip.valPackage = 0;
                            jTemp2.statusTrip = 'Retour sans frais';
                        }
                    }                
    
                    let tab: any = [];
                    tab.push(jTemp2.refTrip, jTemp2.statusTrip, jTemp2.costTrip, jTemp2.destTrip.cityAdr,
                    this.splitDateFormatMDY(jTemp2.getedday), this.splitDateFormatMDY(jTemp2.livredday),
                    cloture, jTemp2.packageTrip.valPackage);
        ...
        () => {
            ...
            this.excelService.generateExcel(this.tripsByUserAndDate, nameuser);
            // this.spinner.hide();
        });
