...
deleteAdresse(adresse) {
  ...
  getLatLngByAddress() {
    console.log((<HTMLInputElement>document.getElementById('gouvernorat')).value);
    console.log((<HTMLInputElement>document.getElementById('delegation')).value);
    console.log($( '#delegation' ).val);
    console.log($( '#gouvernorat' ).val);
    const address = $( '#delegation' ).text() + ' ' + $( '#gouvernorat' ).text();
    console.log('address ', address);
    this.markers = [];
    return Promise.all([this.getLatitudeLongitudeFromAddress(this.showResult, address), this.loadPosByLatLng()]);
    // this.getLatitudeLongitudeFromAddress(this.showResult, address).then(() => this.loadPosByLatLng());
  ...
  loadPosByLatLng() {
    this.latitude = 37.2210031; // this.latitude2;
    this.longitude = -94.7062411; // this.longitude2;
    this.markers = [];
    this.markers.push({
      lat: this.latitude,
      lng: this.longitude,
