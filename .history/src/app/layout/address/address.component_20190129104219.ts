...
deleteAdresse(adresse) {
  ...
  getGovAndDelegOfTunisia() {
      ...
      /* const ajaxJson = this.http.get( 'https://raw.githubusercontent.com/marwein/tunisia/master/tunisia.json')
      .subscribe(data => {
        const result = data['_body'];
        const jo = JSON.parse(result);
        // const jsonOb = Array.of(jo);
        // const jsonOb: any = obj[0];
        const gouvernorat = $( '#gouvernorat' );
        const delegation = $( '#delegation' );
        console.log('data', jo);
        for (let index = 0; index < jo.length; index++) {

          gouvernorat.append('<option value="' + jo[index] + '">' +  jo[index]  + '</option>');
        }
    }); */

    let ajaxJson = jQuery.getJSON( "https://raw.githubusercontent.com/marwein/tunisia/master/tunisia.json")
  .pipe(function( data ) {
      const json = (data.responseJSON);
      const gouvernorat = jQuery( '#gouvernorat' );
        const delegation = jQuery( '#delegation' );
