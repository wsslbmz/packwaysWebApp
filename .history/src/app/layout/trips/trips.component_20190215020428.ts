...
export class TripsComponent implements OnInit {
    ...
    addTripsFromExcelFile() {        				
        ...
        if (this.tripsFromExcel !== null && this.tripsFromExcel.length > 0) {
            for(let i=0; i<this.tripsFromExcel.length; i++) {
                ...
                if ((descriptionTrip === null || descriptionTrip === '') || (valueTrip === null || valueTrip === '' || !valueTrip.match(regex)) 
 ...
 ValidateSingleInput(oInput, contentImport) {
    var _validFileExtensions = [".xlxs", ".xls"];
    if (oInput.type == "file") {
        var sFileName = oInput.value;
         if (sFileName.length > 0) {
            ...
            if (!blnValid) {
                this.snackBar.open("Échec, le fichier est invalide, les extensions autorisées sont: " + _validFileExtensions.join(", "), "Fermer", {
                    duration: 5000,
                });                
                oInput.value = "";
                return false;
