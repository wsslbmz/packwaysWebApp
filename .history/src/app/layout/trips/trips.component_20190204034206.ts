...
export class TripsComponent implements OnInit {
    ...
    getFiltredTripsOnNext() {
        let nextDate = this.items[this.items.length - 1].createdday;
        this.getFiltredTrips(nextDate, this.searchTerm, null);
    }

    getFiltredTripsOnPrevious() {
        let btnPrev = 'previous';
        let prevDate = this.items[0].createdday;
        this.getFiltredTrips(prevDate, this.searchTerm, btnPrev);
    }
