...
export class TripsComponent implements OnInit {
    ...
    PreMessages = ['Numéro incorrect !', 'Client non joignable par téléphone !', 'Client absent au rendez-vous',
    ...
    updatetrip() {
        ...
        this.update(this.objTrip.destTrip.contactAdr, this.objTrip.destTrip.cityAdr, this.objTrip.destTrip.mobileAdr,
             this.objTrip.timingTrip, this.objTrip.statusTrip, this.objTrip.prevStatusTrip,
                    ...
                    this.objTrip.costTrip, this.objTrip.packageTrip.valPackage, this.objTrip.driverTrip);
                    //window.location.reload();
    }
    ...
    showStatusTrip2(trip) {
        this.changeColorByStatusTrip = false;
        this.varcolor = '#ffffff';
        this.dnow = new Date().getTime();
        const dif = Number('172800000');
        if (trip.createdday != null) {
            this.changeColorByStatusTrip = true;
            const dpostulation = new Date(trip.createdday).getTime();
            const d_np = this.dnow - dpostulation;
            if ((d_np >=  dif) && (trip.statusTrip === 'cherche un Livreur')) {
                this.varcolor = '#000000';
            } else if (trip.affectedday != null) {
                const dacceptation = new Date(trip.createdday).getTime();
                const d_na = this.dnow - dacceptation;
                if ((d_na >=  dif) && (trip.statusTrip === 'Livreur en chemin')) {
                    this.varcolor = '#ff1111';
                } else if (trip.getedday != null) {
                    const dramassage = new Date(trip.getedday).getTime();
                    const d_nr = this.dnow - dramassage;
                    if ((d_nr >=  dif) && (trip.statusTrip === 'Chez Livreur')) {
                        this.varcolor = '#ff5555';
                    } else if (trip.startdelday != null) {
                        const ddlivraison = new Date(trip.startdelday).getTime();
                        const d_ndl = this.dnow - ddlivraison;
                        if ((d_ndl >=  dif) && (trip.statusTrip === 'Livraison en cours')) {
                            this.varcolor = '#ff9999';
                        } else if (trip.livredday != null) {
                            const dlivraison = new Date(trip.livredday).getTime();
                            const d_nl = this.dnow - dlivraison;
                            if ((d_nl >=  dif) && (trip.statusTrip === 'Livree')) {
                                this.changeColorByStatusTrip = false;
                                this.varcolor = '#cc8888';
                            } else if (trip.returnedday != null) {
                                const dretour = new Date(trip.returnedday).getTime();
                                const d_nrt = this.dnow - dretour;
                                if ((d_nrt >=  dif) && (trip.statusTrip === 'Retour')) {
                                    this.varcolor = '#bbb777';
                                }
                            }
                        }
                    }
