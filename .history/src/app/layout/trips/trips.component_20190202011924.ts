...
export class TripsComponent implements OnInit {
    ...
    updatetrip() {
        ...
        if ((localStorage.getItem('auth') === 'admin' && stat !== 'cherche un Livreur' && (this.objTrip.driverTrip === null 
            || this.objTrip.driverTrip === '' || this.objTrip.driverTrip === undefined))) {
            this.snackBar.open('Echèc! Veuillez sélectionner le conducteur', 'Fermer', {
                duration: 12000,
            });
            return;
        }
        if (localStorage.getItem('auth') === 'admin' && ((stat === 'Livreur en chemin' && (this.objTrip.affectedday === '' 
         || this.objTrip.affectedday === null))  
