...
export class TripsComponent implements OnInit {
    ...
    getFiltredTripsOnNext() {
        ...
        console.log('nextTripDate', nextTripDate);
        if (this.items.length >= this.chooseView) {
    ...
    getFiltredTripsOnPrevious() {
        ...
        console.log('prevTripDate', prevTripDate);
        if (this.items.length >= this.chooseView) {
            this.prvBtnDisabled = true;
            this.nxtBtnDisabled = true;
    ...
    getLatLngFromGov() {
      ...
      showStatusTrip(trip) {
          ...
          let dif = Number('172800000');

          if (trip.getedday != null) {
            let dramassage = new Date(trip.getedday).getTime();
            let diff = this.dnow - dramassage;
