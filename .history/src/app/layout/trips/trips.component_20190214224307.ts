...
export class TripsComponent implements OnInit {
    ...
    onFileChange(evt: any) {
		...
		reader.onload = (e: any) => {
			...
			/* save data */
            this.tripsFromExcel2 = <AOA>(XLSX.utils.sheet_to_json(ws, {header: 1}));
            this.tripsFromExcel = this.tripsFromExcel2;
    ...
    tripsFromExcelAdrLatLng() {
        if (this.tripsFromExcel !== null && this.tripsFromExcel.length > 0) {
            ...
            for(let i=0; i<this.tripsFromExcel.length; i++) {
                ...
                console.log(city);
                this.getLatitudeLongitudeFromAddress2(this.showResult2, city, i);
            }
        }
    }    

    getLatitudeLongitudeFromAddress2(callback, address, index) {
        address = address || 'Tunis, Tunisie';
        // Initialize the Geocoder
        const geocoder = new google.maps.Geocoder();
        if (geocoder) {
    ...
    showResult2(result, address, index) {
        ...
        this.tripsFromExcel[index][8].push(address);
        this.tripsFromExcel[index][9].push(result.geometry.location.lat());
        this.tripsFromExcel[index][10].push(result.geometry.location.lat());
    }
